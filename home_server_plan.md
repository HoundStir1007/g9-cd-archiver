...
1. **Phase 1A**: Core Document Management
   ...
   Organized backup structure to ensure all important files are properly stored:
   ```
   /HomeServer
   ...
   │
   ├── Media/                   # Media files (less priority for backup)
   │   ├── Photos/              # Family photos (small subset of main collection)
   ...
   └── ServerData/              # Data generated by server applications
       ...
       └── AppData/             # Application-specific data to preserve
   ```
   
   **Sync Priorities:**
   - Full Sync: Documents/, Backups/, Projects/ directories
   - Selective Sync: Media/, ServerData/ (critical files only due to storage constraints)
